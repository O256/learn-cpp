...
// 矢量插值，t取值 [0, 1]
void vector_interp(vector_t *z, const vector_t *x1, const vector_t *x2,
                   float t) {
...
void matrix_add(matrix_t *c, const matrix_t *a, const matrix_t *b) {
  int i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) c->m[i][j] = a->m[i][j] + b->m[i][j];
  }
}
...
void matrix_scale(matrix_t *c, const matrix_t *a, float f) {
  int i, j;
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) c->m[i][j] = a->m[i][j] * f;
  }
}
...
// 设置摄像机
void matrix_set_lookat(matrix_t *m, const vector_t *eye, const vector_t *at,
                       const vector_t *up) {
...
void vertex_interp(vertex_t *y, const vertex_t *x1, const vertex_t *x2,
                   float t) {
...
void vertex_division(vertex_t *y, const vertex_t *x1, const vertex_t *x2,
                     float w) {
  float inv = 1.0f / w;
...
void trapezoid_init_scan_line(const trapezoid_t *trap, scanline_t *scanline,
                              int y) {
...
// 设置当前纹理
void device_set_texture(device_t *device, void *bits, long pitch, int w,
                        int h) {
  char *ptr = (char *)bits;
  int j;
...
// 绘制线段
void device_draw_line(device_t *device, int x1, int y1, int x2, int y2,
                      IUINT32 c) {
  int x, y, rem = 0;
  if (x1 == x2 && y1 == y2) {
...
int screen_init(int w, int h, const TCHAR *title) {
  ...
  screen_handle =
      CreateWindow(_T("SCREEN3.1415926"), title,
                   ...
                   0, 0, 0, NULL, NULL, wc.hInstance, NULL);
...
static LRESULT screen_events(HWND hWnd, UINT msg, WPARAM wParam,
                             LPARAM lParam) {
  switch (msg) {
    case WM_CLOSE:
      screen_exit = 1;
...
int main(void) {
  ...
  TCHAR *title =
      _T("Mini3d (software render tutorial) - ")
  ...
  while (screen_exit == 0 && screen_keys[VK_ESCAPE] == 0)  // 主循环
  {
    ...
    if (screen_keys[VK_UP])     // 上键
      pos -= 0.01f;             // 位置
    if (screen_keys[VK_DOWN])   // 下键
      pos += 0.01f;             // 位置
    if (screen_keys[VK_LEFT])   // 左键
      alpha += 0.01f;           // 旋转角度
    if (screen_keys[VK_RIGHT])  // 右键
      alpha -= 0.01f;           // 旋转角度
    ...
    if (screen_keys[VK_SPACE]) {
      if (kbhit == 0) {
        ...
        device.render_state = states[indicator];  // 渲染状态
      }
    } else {
      kbhit = 0;
    }
